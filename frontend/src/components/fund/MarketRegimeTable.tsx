...
export function MarketRegimeTable({ regimes }: { regimes: RegimeBehavior[] }) {
  return (
    <div className="space-y-3">
      {regimes.map((r) => (
        <div key={r.regime_name} className="rounded-lg border border-neutral-800 p-4">
            ...
            {r.fund_available && (
              <span className={`text-sm font-medium ${signColorClass(r.fund_return_pct)}`}>
                ...
                {r.benchmark_available && (
                  <span className="text-neutral-500 font-normal"> vs {formatPct(r.benchmark_return_pct)}</span>
                )}
              </span>
            )}
          </div>
          <p className="text-sm text-neutral-400">{r.summary}</p>
        </div>
      ))}
    </div>
  );

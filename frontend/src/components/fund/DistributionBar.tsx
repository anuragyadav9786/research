...
export function DistributionBar({ distribution }: { distribution: RollingReturnDistribution }) {
  ...
  if (min === null || max === null || min === max) {
    return <p className="text-sm text-neutral-500">Not enough history to show a distribution.</p>;
  }
  ...
  return (
    <div className="space-y-2">
      <div className="relative h-8 rounded bg-neutral-900 border border-neutral-800 overflow-hidden">
        <div
          className="absolute top-0 bottom-0 bg-neutral-700/50"
          style={{ left: `${toPct(p10 ?? min)}%`, right: `${100 - toPct(p90 ?? max)}%` }}
          title={`P10–P90: ${formatPct(p10)} to ${formatPct(p90)}`}
        />
        <div
          className="absolute top-0 bottom-0 bg-cyan-800/70"
        ...
        {showZeroLine && (
          <div className="absolute top-0 bottom-0 w-px bg-neutral-400" style={{ left: `${toPct(0)}%` }} title="0%" />
        )}
      </div>
      <div className="flex justify-between text-xs text-neutral-500">
        <span>worst {formatPct(min)}</span>
        <span className="text-neutral-300">median {formatPct(median)}</span>

...
guessing at: "Growth Option", "GROWTH", "Growth" all mean growth; "IDCW
Option", "IDCW", "IDCW-Re-investment", "MONTHLY DCW Payout", "QUARTERLY
IDCW Payout" (verified against the live file) all mean IDCW (AMFI's own
data uses "DCW" as a shorthand for "IDCW" in some payout-frequency
variants). Anything this can't confidently classify as exactly one of
growth/IDCW — including a blank Plan/Option, which is typical of
exchange-traded instruments that carry no distributor-plan concept at
all — returns None rather than guessing, per Rule 2 (never fabricate
financial data): a wrong guess here would misclassify a real fund's
identity, not just omit a data point.
"""
...
_IDCW_PATTERN = re.compile(r"\b(IDCW|Dividend|DCW)\b", re.IGNORECASE)
...
def parse_option(option_raw: str) -> str | None:
    """Classify an Option column value as 'growth' or 'idcw', or None if
    neither (or both) are recognizable — e.g. a blank Option field, or an
    option this schema doesn't model (like "Bonus")."""

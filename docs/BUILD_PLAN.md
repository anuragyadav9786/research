...
3. **Benchmark data source** — NSE index data availability/licensing for free use should be confirmed before Phase 3. **Update (Phase 17):** the benchmark data engine (schema, lazy fetch/cache, provider abstraction) is built and wired end-to-end, with an `NSEProvider` implementation against `nseindia.com`'s own `indicesHistory` API, now enabled (`Settings.benchmark_nse_provider_enabled = true`). Its response *shape* is confirmed against a real captured sample; still outstanding: the request side (session cookies, headers, query params) has never been confirmed against a live round trip (every development environment so far has had no outbound network access — an explicit 403 policy denial, not a transient failure), whether a TRI-named query actually returns TRI values through this endpoint, and its terms of use for this kind of automated fetch haven't been separately confirmed. Enabling it anyway was a deliberate choice — a failed or malformed fetch fails closed to "Data unavailable," never a wrong number — but this open decision isn't fully resolved. See `docs/data-sources.md` section 4.
